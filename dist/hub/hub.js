...
export class Hub {
    ...
    subscribe(port, option = 0, callback) {
        ...
        this.write(
        // @ts-ignore
        Buffer.from([0x0a, 0x00, 0x41, portNum, option, 0x01, 0x00, 0x00, 0x00, 0x01]), callback);
    }
    /**
    ...
    unsubscribe(port, option = 0, callback) {
        ...
        this.write(
        // @ts-ignore
        Buffer.from([0x0a, 0x00, 0x41, portNum, option, 0x01, 0x00, 0x00, 0x00, 0x00]), callback);
    }
    subscribeAll() {
    ...
    encodeLed(color) {
        ...
        const colorNum = typeof color === 'string' ? this.ledColors.indexOf(color) : color;
        // @ts-ignore
